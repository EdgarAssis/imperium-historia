...
export function renderTopbar(state) {
  ...
  qs('#stat-currency').textContent = `${state.currency.strength}`;
  qs('#stat-inflation').textContent = `${state.currency.inflation}%`;
  qs('#stat-currency').textContent = state.currency?.strength ?? 0;
  qs('#stat-inflation').textContent = `${state.currency?.inflation ?? 0}%`;
  qs('#stat-interest').textContent = `${state.currency?.interestRate ?? 0}%`;
  const inflation = state.currency?.inflation ?? 0;

  if (inflation > 10) {
    qs('#stat-inflation').style.color = 'red';
  } else if (inflation > 5) {
    qs('#stat-inflation').style.color = 'orange';
  } else {
    qs('#stat-inflation').style.color = 'lightgreen';
  }
}
